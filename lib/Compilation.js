...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		// ===== 模块处理的分层队列系统 =====
		// Webpack 使用分层的 AsyncQueue 系统来管理复杂的模块处理流程：
		// 1. 每个队列负责特定的处理阶段，实现职责分离和错误隔离
		// 2. 通过 parent 属性建立父子关系，实现优先级管理和资源共享
		// 3. 子队列会共享父队列的并行度，并且父队列优先处理
		// 4. 处理流程：factorize → add → processDependencies → build
		// 5. 这种设计可以提高并发性能，同时保证处理顺序的正确性
	...
	_handleModuleBuildAndDependencies(
		...
		this.buildModule(module, (err) => {
			...
			/**
			 * 处理当前 module 下面的依赖列表，都处理成module
			 */
	...
	seal(callback) {
		...
		// module: 模块，不同的资源文件
		// chunk： 一个或多个 module 组成
		// chunkGroup：一个或多个chunk 组成。
		// moduleGraph：各个 module之间的依赖关系，生成chunkGraph用到
		// ChunkGraph： 用于储存 ChunkGraph module、chunk、chunkGroup 三者之间的关系
					...
					this.codeGeneration((err) => {
						...
						this._runCodeGenerationJobs(codeGenerationJobs, (err) => {
							...
							const cont = () => {
								...
								// 调用钩子，在TerserPlugin会对注释进行处理，使得产物更加清爽
