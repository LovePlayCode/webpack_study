 ...
 * 发起N个并行任务，每个任务完成时都会调用同一个回调，当所有 N 个都完成时执行 callback(null)；但如果任一任务中途发生错误，立即调用 callback，阻止后续再调用一次最终回调。
 */
	...
	constructor({
		...
		this.hooks.factorize.tapAsync(
			...
			(resolveData, callback) => {
				// 这里通过调用 callAsync 来执行注册在resolve上的方法，从而构造出一个resolveData，主要是使用了Object.assign(data.create...) 可以看一下这部分。
					...
					this.hooks.afterResolve.callAsync(resolveData, (err, result) => {
						...
						this.hooks.createModule.callAsync(
							...
							(err, createdModule) => {
								...
								// 这里会调用SideEffectsFlagPlugin 插件注册的方法，对 TreeShaking 进行优化
			...
			(data, callback) => {
				...
				const continueCallback = needCalls(2, (err) => {
					...
					if (
						...
						// 根据配置(如 webpack.config.js)的规则找到对应的 loader。 result 就是最终找到的 loader
						...
						for (const r of result) {
							// Rule.enforce 对应的是处理这个属性
							// 相关中文文档：https://webpack.docschina.org/configuration/module/#ruleenforce
					...
					const continueCallback = needCalls(3, (err) => {
						...
						try {
							// 将找到的相关组件(比如 loader，parse 等)合并到 data.createData,为后续创建 module 做准备
					...
					this.resolveRequestArray(
						...
						resolveContext,
						(err, result) => {
							// 通过 resolve 处理，将 result 的结果赋值给normalLoaders，同时开启新的continueCallback
