...
class AsyncQueue {
	...
	add(item, callback) {
		...
		this.hooks.beforeAdd.callAsync(item, (err) => {
			...
			// 把 item 映射为唯一 key
			// 在 webpack 场景：应当用模块的唯一标识符（例如 module.identifier() / module.resource + loaderQuery + resourceQuery），否则可能错误合并不同配置下的模块。
	...
	_startProcessing(entry) {
		this.hooks.beforeStart.callAsync(entry.item, (err) => {
			...
			try {
				// 这里是_factorizeModule(不一定是，得看构造器传入的参数是什么)，会调用这个回调。webpack 走的是事件注册监听机制，所以 callback 特别多。
				// 各种回调里会创建出一个 module， r = factorFactory = {...,module}一个复合对象。
	 ...
	 * @returns {void}
	 * 在 hooks.result 中可以再对结果处理或改写错误。将 entry.state 置为 DONE，并把 entry.result/entry.error 保存下来，减少后续重复处理。随后把所有回调在合适的事件循环时机（同步或 nextTick）执行，并恢复 root._activeTasks 并可能触发下一轮处理。
	 */
	_handleResult(entry, err, result) {
		this.hooks.result.callAsync(entry.item, err, result, (hookError) => {
			...
			const root = this._root;
			root._activeTasks--;
			// 把activeTasks-1,调用_ensureProcessing，当前任务执行完毕，看看有没有别的任务要干。
			if (root._willEnsureProcessing === false && root._needProcessing) {
				root._willEnsureProcessing = true;
				setImmediate(root._ensureProcessing);
			}
			// 回调嵌套太多，用process.nextTick 延迟执行，避免栈溢出。
			if (inHandleResult++ > 3) {
				process.nextTick(() => {
